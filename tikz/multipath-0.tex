% Requires:
% \usetikzlibrary{patterns.meta}
% \usetikzlibrary{shapes.misc}
\begin{tikzpicture}[
    scale=1.0,
    wall/.style={draw=red, thick},
    ground/.style={fill,draw=none,minimum width=0.3,minimum height=0.6},
  ]
  \colorlet{los_color}{green!50}
  \colorlet{rup_color}{red!50}
  \colorlet{rdw_color}{blue!50}
  \coordinate (SW) at (-0.5, -1.5);
  \coordinate (NE) at (+2.5, +1.5);
  \coordinate (TX) at (0, 0);

  % Upper wall (slanted up)
  \coordinate (S1) at (0.5, +0.9);
  \coordinate (E1) at (1.5, +1.1);

  % Lower wall (slanted up, parallel to the top wall)
  \coordinate (S2) at (0.2, -1.0);
  \coordinate (E2) at (1.2, -0.8);

  \clip[rounded corners] (SW) rectangle (NE);

  \begin{scope}
    % LOS region updated for the new wall coordinates
    \clip (S1) -- (20, 36) -- (-20, 36) -- (-20, -100) -- (20, -100) -- (S2) -- (E2) -- (20, -13.3) -- (20, 14.7) -- (E1) -- cycle;
    \fill[los_color,fill opacity=.5] (SW) rectangle (NE);
  \end{scope}

  \begin{scope}
    % Upper wall reflection (Rays pass safely above the lower wall)
    \clip (S1) -- (20, -14.5) -- (20, -3.4) -- (E1) -- cycle;
    \fill[rup_color,fill opacity=.5] (SW) rectangle (NE);
  \end{scope}

  \begin{scope}
    % Lower wall reflection WITH the shadow cone cut out.
    % The path goes up the left beam edge, routes down the left shadow edge to S1,
    % travels across the bottom of the wall to E1, goes up the right shadow edge,
    % and finally traces down the right beam edge.
    \clip (S2) -- (-4.0, 20) -- (1.16, 20) -- (S1) -- (E1) -- (8.21, 20) -- (15.06, 20) -- (E2) -- cycle;
    \fill[rdw_color,fill opacity=.5] (SW) rectangle (NE);
  \end{scope}

  \draw[dashed, ultra thick, rounded corners] (SW) rectangle (NE);
  \draw[ground,pattern={Lines[angle={45-atan(0.2)}]},rotate around={atan(0.2):(S1)}] (S1) rectangle ++({1},+.1);
  \draw[wall] (S1) -- (E1);
  \draw[ground,pattern={Lines[angle={45-atan(0.2)}]},rotate around={atan(0.2):(S2)}] (S2) rectangle ++({1},-.1);
  \draw[wall,blue] (S2) -- (E2);
  \node[draw, cross out, thick, inner sep=2pt] (TX) {};
\end{tikzpicture}
